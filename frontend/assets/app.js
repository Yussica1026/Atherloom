...
function renderGameRoom(){const current=currentGameSave();if(!current||!gameState.current)return;const name=activePersonaName(),messages=current.room_messages||[];$("#gameRoom").hidden=false;$("#gameRoomTitle").textContent=`和 ${name} 边玩边聊`;$("#gameRoomThoughtName").textContent=`${name}的心里话`;$("#gameRoomThought").textContent=current.last_thought||"接过这一回合后，想法会留在这里。";$("#gameRoomContext").textContent=`正在一起玩「${gameNames[gameState.current]}」· 房间跟随真实局面`;$("#gameRoomMessages").innerHTML=messages.map(item=>`<div class="game-room-message ${item.role}">${escapeHtml(item.content)}</div>`).join("")||`<div class="game-room-empty">你们还没说话。先玩一步，或者叫一声他的名字。</div>`;$("#gameRoomMessages").scrollTop=$("#gameRoomMessages").scrollHeight;}
function renderClaw(){const current=gameState.claw;if(!current)return;$("#clawCoins").textContent=current.coins;$("#clawTurns").textContent=current.turn;$("#clawHead").style.left=`${current.position*20+10}%`;$("#clawPrizes").innerHTML=current.prizes.map((name,index)=>`<span class="${index===current.position?"targeted":""}">◇<small>${escapeHtml(name)}</small></span>`).join("");$("#clawInventory").innerHTML=Object.entries(current.inventory).map(([name,count])=>`<span><b>${escapeHtml(name)}</b><em>× ${count}</em></span>`).join("")||"<small>还没有抓到娃娃。</small>";$("#clawJournal").innerHTML=[...current.journal].reverse().slice(0,8).map(item=>`<span>${escapeHtml(item)}</span>`).join("")||"<small>机器正在等待第一爪。</small>";renderGameRoom();}
...
async function prepareChatGameContext(content){
  ...
  const {gameId,turns}=request,name=activePersonaName();
  try{
...
$$("[data-maze-move]").forEach(button=>button.onclick=()=>playMiniGame("mist_maze",button.dataset.mazeMove));
$("#resetDungeon").onclick=()=>playMiniGame("ember_dungeon","reset");
$$("[data-dungeon-action]").forEach(button=>button.onclick=()=>playMiniGame("ember_dungeon",button.dataset.dungeonAction));
...
async function sendGameRoomMessage(){const provider=activeProvider()||state.providers[0],input=$("#gameRoomInput"),content=input.value.trim(),button=$("#sendGameRoomMessage");if(!content||button.disabled)return;if(!provider){$("#gameRoomMessages").insertAdjacentHTML("beforeend",'<div class="game-room-message event">还没有可用线路。游戏没有退出；请先到设置里添加一条线路。</div>');return;}state.provider||=provider.id;button.disabled=true;button.textContent="等待回复…";input.value="";$("#gameRoomMessages").insertAdjacentHTML("beforeend",`<div class="game-room-message user" data-room-pending>${escapeHtml(content)}</div><div class="game-room-message event" data-room-waiting>${escapeHtml(activePersonaName())} 正在看当前局面，你仍然可以继续玩。</div>`);$("#gameRoomMessages").scrollTop=$("#gameRoomMessages").scrollHeight;try{const gameId=gameState.current,payload=await api(`/api/games/${gameId}/room-chat`,{method:"POST",body:JSON.stringify({provider_id:provider.id,persona_id:state.persona,content}),timeout:20000});storeGameState(gameId,payload.state);renderCurrentGame(gameId);}catch(error){$("[data-room-waiting]")?.remove();$("[data-room-pending]")?.remove();$("#gameRoomMessages").insertAdjacentHTML("beforeend",`<div class="game-room-message user">${escapeHtml(content)}</div><div class="game-room-message event">回复失败：${escapeHtml(error.message)}。棋局没有变化，可以重试。</div>`);$("#gameRoomMessages").scrollTop=$("#gameRoomMessages").scrollHeight;}finally{button.disabled=false;button.textContent="发送";input.focus();}}
