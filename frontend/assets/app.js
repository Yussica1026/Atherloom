...
function renderHomestead(){const current=gameState.homestead,catalog=gameState.homesteadCatalog;if(!current||!catalog)return;$("#homesteadCoins").textContent=current.coins;$("#homesteadAiEnabled").checked=!!current.management?.enabled;$("#homesteadManagerStatus").textContent=current.management?.enabled?`已授权 ${activePersonaName()} 管理 · 每日预算 ${current.management.daily_budget} 云贝`:"现在由你亲自照料";$("#homesteadGarden").innerHTML=current.garden.map((plant,index)=>{if(!plant)return `<article class="flower-pot empty"><div class="pot-soil"></div><strong>空花盆</strong><div class="seed-menu">${Object.entries(catalog.flowers).map(([species,item])=>`<button data-plant="${species}" data-pot="${index}"><span class="pixel-flower tiny" data-species="${species}"></span>${escapeHtml(item.name)} · ${item.seed_cost}</button>`).join("")}</div></article>`;const status={growing:"正在长大",blooming:"开花了",wilted:"有些枯萎",dead:"已经枯死"}[plant.status];return `<article class="flower-pot ${plant.status}"><span class="pixel-flower" data-species="${plant.species}"></span><strong>${escapeHtml(plant.name)}</strong><small>${status} · 成长 ${Math.round(plant.growth)}%</small><div class="plant-bars"><i style="--value:${plant.moisture}%">水分</i><i style="--value:${plant.vitality}%">生命</i></div><div class="flower-actions">${plant.status==="dead"?`<button data-home-action="clear_plant" data-pot="${index}">清理花盆</button>`:`<button data-home-action="water" data-pot="${index}">浇水</button><button data-home-action="fertilize" data-pot="${index}">施肥</button>${plant.growth>=100?`<button data-home-action="harvest" data-pot="${index}">收花</button>`:""}`}</div></article>`;}).join("");document.querySelectorAll("[data-plant]").forEach(button=>button.onclick=()=>playHomestead({action:"plant",target:Number(button.dataset.pot),species:button.dataset.plant}));document.querySelectorAll("[data-home-action]").forEach(button=>button.onclick=()=>playHomestead({action:button.dataset.homeAction,target:Number(button.dataset.pot)}));const pet=current.pet;$("#petEmpty").hidden=!!pet;$("#petDetails").hidden=!pet;$("#petAvatar").className=`pet-avatar${pet?` pixel-pet mood-${pet.mood}`:""}`;if(pet)$("#petAvatar").dataset.kind=pet.kind;else delete $("#petAvatar").dataset.kind;if(!pet){$("#petAdoption").innerHTML=Object.entries(catalog.pets).map(([kind,item])=>`<button data-adopt="${kind}"><span class="pixel-pet" data-kind="${kind}"></span>${escapeHtml(item.name)}</button>`).join("");document.querySelectorAll("[data-adopt]").forEach(button=>button.onclick=()=>{const fallback=catalog.pets[button.dataset.adopt].name,name=prompt("给它取个名字",fallback);if(name!==null)playHomestead({action:"adopt",kind:button.dataset.adopt,name:name.trim()||fallback});});}else{$("#petName").textContent=pet.name;$("#petMood").textContent=homesteadMoodLabel(pet.mood);const meters={hunger:["饱腹",pet.hunger],happiness:["开心",pet.happiness],hygiene:["清洁",pet.hygiene],energy:["精力",pet.energy]};$("#petMeters").innerHTML=Object.values(meters).map(([label,value])=>`<label><span>${label}</span><i><b style="width:${value}%"></b></i><em>${Math.round(value)}</em></label>`).join("");$("#petSkills").innerHTML=Object.entries(pet.skills||{}).map(([key,value])=>`<span>${catalog.school_subjects[key]} ★${value}</span>`).join("")||"<small>还没有上过课。</small>";}$("#homesteadInventory").innerHTML=`<span>宠物粮 × ${current.inventory.pet_food||0}</span><span>肥料 × ${current.inventory.fertilizer||0}</span><span>洗护 × ${current.inventory.soap||0}</span>`;$("#homesteadEvents").innerHTML=[...(current.events||[])].reverse().slice(0,12).map(item=>`<span>${escapeHtml(item.text)}</span>`).join("");}
...
async function openGame(gameId) {
  ...
  $("#gameEmpty").hidden=true;$("#homesteadStage").hidden=gameId!=="homestead";$("#fishingStage").hidden=gameId!=="quiet_fishing";$("#clawStage").hidden=gameId!=="claw_machine";$("#slotsStage").hidden=gameId!=="cloud_slots";$("#starMergeStage").hidden=gameId!=="star_merge";$("#mazeStage").hidden=gameId!=="mist_maze";$("#dungeonStage").hidden=gameId!=="ember_dungeon";
...
$("#memoryForm").onsubmit = async e => { e.preventDefault(); const form = e.target; const data = {...Object.fromEntries(new FormData(form)),persona_key:memoryPersonaKey()}; const editing = form.dataset.editing; const saved = await api(editing ? `/api/memories/${editing}` : "/api/memories", { method: editing ? "PUT" : "POST", body: JSON.stringify(data) }); if (editing) state.memories=state.memories.filter(item=>item.id!==editing||saved.persona_key===memoryListKey()); else if(saved.persona_key===memoryListKey())state.memories.unshift(saved); form.reset(); delete form.dataset.editing; $("#saveMemory").textContent = "添加到当前人格"; $("#cancelMemoryEdit").hidden = true; renderSettings(); };
