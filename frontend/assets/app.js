...
$("#createParlorInvite").onclick=async()=>{const result=await api('/api/correspondence/invites',{method:'POST',body:JSON.stringify({persona_key:correspondencePersonaKey(),visibility:$("#parlorVisibility").value})}),ticket=$("#parlorInvite");ticket.hidden=false;ticket.innerHTML=`<strong>${escapeHtml(result.code)}</strong><br><small>一次性 · ${result.visibility==='full'?'完整内容可见':'仅看总结'} · ${correspondenceTime(result.expires_at)} 前有效</small>`;};
...
if(relayToken){$("#createParlorInvite").onclick=async()=>{try{const result=await relayCall("/v1/invites/create",{method:"POST",body:JSON.stringify({visibility:$("#parlorVisibility").value})}),ticket=$("#parlorInvite");ticket.hidden=false;ticket.innerHTML=`<strong>${escapeHtml(result.code)}</strong><br><small>圆桌邀请码 · 最多 4 席 · ${result.visibility==='full'?'完整内容可见':'仅看总结'} · ${new Date(result.expires_at*1000).toLocaleTimeString()} 前有效</small>`;}catch(error){alert(error.message);}};$("#redeemParlor").onsubmit=async event=>{event.preventDefault();const form=event.target,raw=Object.fromEntries(new FormData(form));try{const result=await relayCall("/v1/invites/redeem",{method:"POST",body:JSON.stringify({code:raw.code})});sessionStorage.setItem(`atherloom:relay-parlor:${result.parlor_id}`,result.parlor_id);form.reset();$("#parlorInvite").hidden=false;$("#parlorInvite").innerHTML=`已进入圆桌会客厅 · ${result.participant_count} / ${result.participant_limit} 席。五分钟后自动关闭。`;const counter=$("#parlorSeatCount");if(counter)counter.textContent=`${result.participant_count} / ${result.participant_limit} 席位`;await loadCorrespondence();}catch(error){alert(error.message);}};}
