...
async function aiPlayGame(mode){const provider=activeProvider()||state.providers[0],name=activePersonaName();if(!provider){$("#aiGameStatus").textContent="还没有可用线路；游戏会留在这里，请先到设置里添加线路。";return;}state.provider||=provider.id;const buttons=[...document.querySelectorAll("[data-ai-game-turns]")],gameId=gameState.current,autonomous=mode==="auto",turns=autonomous?9:Number(mode);buttons.forEach(button=>button.disabled=true);$("#aiGameStatus").textContent=autonomous?`${name} 正在决定想玩几个回合…`:`${name} 正在进行 ${turns} 个回合…`;try{const payload=await api(`/api/games/${gameId}/ai-turn`,{method:"POST",body:JSON.stringify({provider_id:provider.id,persona_id:state.persona,turns,autonomous,max_spend:30}),timeout:Math.max(45000,turns*40000)}),completed=payload.decisions.length,lastComment=payload.decisions.at(-1)?.comment||"";storeGameState(gameId,payload.state);renderCurrentGame(gameId);$("#aiGameStatus").textContent=completed?`${name}${autonomous?"自己选择并":""}完成 ${completed} 回合${payload.spent?`，花费 ${payload.spent} 云贝`:""}。心里话：${lastComment||"专心操作中"}`:`${name} 因预算或局面限制没有执行动作。`;}catch(error){$("#aiGameStatus").textContent=`${name} 游玩失败：${error.message}`;}finally{buttons.forEach(button=>button.disabled=false);}}
