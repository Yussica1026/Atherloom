...
const gameState = { catalog: [], current: null, fishing: null, claw: null, slots: null, starMerge: null, starMergeMode: "self", waters: {} };
...
function renderStarMerge(){const current=gameState.starMerge;if(!current)return;const self=gameState.starMergeMode==="self",thought=current.last_thought||"",personaName=activePersonaName();$("#starMergeScore").textContent=current.score;$("#starMergeBest").textContent=current.best;$("#starMergeTurns").textContent=current.turn;$("#starMergeStatus").textContent=current.status==="won"?"已经合成 2048":current.status==="over"?"本局结束":"进行中";$("#starMergeBoard").innerHTML=current.board.map(value=>`<span class="star-tile" data-value="${value||0}">${value||""}</span>`).join("");$("#starMergeJournal").innerHTML=[...(current.journal||[])].reverse().slice(0,8).map(item=>`<span>${escapeHtml(item)}</span>`).join("")||"<small>移动一次，星潮就会开始留下记录。</small>";$("#starMergeThought").textContent=thought||"等我接过棋盘时，会把想到的告诉你。";$("#starThoughtName").textContent=`${personaName}的想法`;$("#starGoalName").textContent=`${personaName}的目标`;$("#starModeSelf").classList.toggle("active",self);$("#starModeAi").classList.toggle("active",!self);$("#starDirectionPad").hidden=!self;$("#starTurnOwner").textContent=self?"现在由你掌舵":`现在交给 ${personaName}`;$("#undoStarMerge").disabled=!(current.history||[]).length;if(gameState.current==="star_merge")$("#aiGameControls").hidden=self;}
