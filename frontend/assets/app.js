...
function renderMcpAudit(){const list=$("#mcpAuditList");if(!list)return;list.innerHTML=state.mcp_audit.map(item=>`<div class="list-card"><div><strong>${escapeHtml(item.server_name||"已删除服务")} · ${escapeHtml(item.tool_name)}</strong><small>${escapeHtml(item.status)} · ${new Date(item.created_at).toLocaleString()}</small>${item.detail?`<p>${escapeHtml(item.detail)}</p>`:""}</div></div>`).join("")||`<p class="muted">还没有 MCP 调用记录。</p>`;}
...
async function loadInnerWriting(){
  ...
  $("#boardList").innerHTML=state.board_messages.map(item=>`<article class="board-note ${item.author==="ai"?"from-ai":"from-user"}"><span>${item.author==="ai"?"AI 留言":"我的留言"}</span><p>${escapeHtml(item.content)}</p><footer>${new Date(item.created_at).toLocaleString()} · ${item.visible_to_ai?"AI 可读":"不提供给 AI"} <button data-delete-board="${item.id}">移除</button></footer></article>`).join("")||`<p class="muted">留言板还是空的。</p>`;
...
function renderRoleplayBackstage(){const story=roleplayState.current,log=$("#roleplayBackstageLog");if(!story||!log)return;log.innerHTML=(story.backstage||[]).length?story.backstage.map(item=>`<article class="${item.role==="user"?"from-author":"from-ai"}"><b>${item.role==="user"?"作者":"AI 本体"}</b><p>${escapeHtml(item.content)}</p></article>`).join(""):`<p class="roleplay-backstage-empty">幕布后面还很安静。可以直接问人物动机、下一幕走向，或要求调整写法。</p>`;requestAnimationFrame(()=>log.scrollTop=log.scrollHeight);}
function renderRoleplayStage(){
  const story=roleplayState.current;if(!story)return;
  $("#roleplaySetup").hidden=true;$("#roleplayStage").hidden=false;$("#roleplayTitle").textContent=story.title;$("#roleplayPlayer").textContent=`你是 ${story.player_name} · 登场：${story.cast.map(actor=>actor.name).join("、")||"尚未填写"}`;
  ...
  $("#roleplayTurnForm").hidden=story.status==="completed"||roleplayState.busy||!story.turns.length;$("#retryRoleplayOpening").hidden=roleplayState.busy||story.turns.length>0;
...
async function requestRoleplayOpening(mode="ai",prose=""){const story=roleplayState.current;if(!story||roleplayState.busy)return;roleplayState.busy=true;$("#roleplayStatus").textContent=mode==="manual"?"正在保存你的开场…":"旁白正在写开场…";renderRoleplayStage();try{const opening=await api(`/api/roleplay/stories/${story.id}/opening${mode==="manual"?"/manual":""}`,{method:"POST",body:JSON.stringify(mode==="manual"?{prose}:{}),timeout:150000});if(!story.turns.some(turn=>turn.turn_number===0))story.turns.push(opening);story.state={...story.state,scene:opening.checkpoint.scene,rolling_summary:`开场：${opening.prose}`};$("#roleplayStatus").textContent="开场已经保存";}catch(error){$("#roleplayStatus").textContent=`开场失败：${error.message}`;}finally{roleplayState.busy=false;renderRoleplayStage();}}
...
async function rerollRoleplayTurn(turnNumber){const story=roleplayState.current,turn=story?.turns.find(item=>item.turn_number===turnNumber);if(!turn||roleplayState.busy)return;const later=story.turns.filter(item=>item.turn_number>turnNumber).length;if(!confirm(later?`重写这一幕会同时撤回后面的 ${later} 回合，继续吗？`:`重新生成${turnNumber===0?"开场":`第 ${turnNumber} 回合`}？`))return;roleplayState.busy=true;renderRoleplayStage();$("#roleplayStatus").textContent=turnNumber===0?"旁白正在重写开场…":"正在重写这一回合…";try{await truncateRoleplayFrom(turnNumber);roleplayState.busy=false;if(turnNumber===0){await requestRoleplayOpening("ai");return;}roleplayState.busy=true;const replacement=await api(`/api/roleplay/stories/${story.id}/turns`,{method:"POST",body:JSON.stringify({player_input:turn.player_input}),timeout:360000});story.turns.push(replacement);story.state={...story.state,turn_number:replacement.turn_number,scene:replacement.checkpoint.scene,rolling_summary:replacement.checkpoint.rolling_summary||story.state.rolling_summary};$("#roleplayStatus").textContent=`第 ${turnNumber} 回合已经重写`;}catch(error){$("#roleplayStatus").textContent=`重 Roll 失败：${error.message}`;await loadRoleplayStory(story.id);}finally{roleplayState.busy=false;renderRoleplayStage();}}
...
$("#roleplaySetup").onsubmit=async event=>{
  ...
  button.disabled=true;button.textContent="正在建立故事…";
  try{const opening_mode=form.elements.opening_mode.value,opening_text=form.elements.opening_text.value.trim();if(!form.dataset.editing&&opening_mode==="manual"&&!opening_text)throw new Error("选择“我来写”后，请先写下开场白");const payload={title:form.elements.title.value.trim(),player_name:form.elements.player_name.value.trim(),premise:form.elements.premise.value.trim(),preset,worldbook_ids,narrator_provider_id:form.elements.narrator_provider_id.value,cast,opening_mode},editing=form.dataset.editing;roleplayState.current=await api(editing?`/api/roleplay/stories/${editing}`:"/api/roleplay/stories",{method:editing?"PUT":"POST",body:JSON.stringify(payload)});if(editing){const index=roleplayState.stories.findIndex(item=>item.id===editing);if(index>=0)roleplayState.stories[index]=roleplayState.current;renderRoleplayStage();$("#roleplayStatus").textContent="故事设置已保存，从下一回合生效";}else{roleplayState.stories.unshift(roleplayState.current);renderRoleplayStage();await requestRoleplayOpening(opening_mode,opening_text);}}catch(error){alert(error.message);}finally{button.disabled=false;button.textContent="保存并开场";}
};
...
$("#roleplayBackstageForm").onsubmit=async event=>{event.preventDefault();const story=roleplayState.current,input=event.target.querySelector("textarea"),button=event.target.querySelector("button"),content=input.value.trim();if(!story||!content)return;button.disabled=true;$("#roleplayBackstageStatus").textContent=`${activePersonaName()}正在重读当前剧情…`;try{const result=await api(`/api/roleplay/stories/${story.id}/backstage`,{method:"POST",body:JSON.stringify({content,provider_id:activeProvider()?.id,persona_id:state.persona}),timeout:150000});story.backstage=result.backstage;input.value="";renderRoleplayBackstage();$("#roleplayBackstageStatus").textContent="这段讨论只留在幕后，不进入正文";}catch(error){$("#roleplayBackstageStatus").textContent=`讨论失败：${error.message}`;}finally{button.disabled=false;}};
...
window.addEventListener("atherloom:roleplay-progress",event=>{if(event.detail?.storyId===roleplayState.current?.id)$("#roleplayStatus").textContent=event.detail.phase;});
...
["openReading","openCinema","openRoleplay"].forEach(id=>document.getElementById(id)?.addEventListener("click",()=>{if(innerWidth<=760)setSidebar(false);}));
...
document.querySelectorAll("[data-inner-space]").forEach(button=>button.onclick=()=>{document.querySelectorAll("[data-inner-space]").forEach(item=>item.classList.toggle("active",item===button));$("#journalSpace").hidden=button.dataset.innerSpace!=="journal";$("#boardSpace").hidden=button.dataset.innerSpace!=="board";});
