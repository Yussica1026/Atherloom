...
function openManualCompress(){if(!state.current){alert("当前还没有可压缩的对话");return;}const available=Math.max(0,state.messages.filter(item=>item.role==="user").length-1),input=$("#manualCompressRounds");input.max=String(Math.max(1,Math.min(100,available)));input.value=String(Math.max(1,Math.min(10,available)));$("#manualCompressHint").textContent=available?`当前最多可选择约 ${available} 轮；始终保留最近一轮原文。`:"当前没有可压缩的旧轮次。";$("#manualCompressStatus").textContent="";$("#confirmManualCompress").disabled=!available;$("#manualCompressDialog").hidden=false;input.focus();}
...
$("#manualCompressDialog form").onsubmit=async event=>{event.preventDefault();if(currentBusy()){$("#manualCompressStatus").textContent="请先等待当前回复完成或停止生成。";return;}const button=$("#confirmManualCompress"),rounds=Number($("#manualCompressRounds").value),provider=activeProvider();if(!provider){$("#manualCompressStatus").textContent="请先给当前对话选择模型线路。";return;}button.disabled=true;button.textContent="正在压缩…";$("#manualCompressStatus").textContent="模型正在整理较早对话，请稍候。";try{const result=await api(`/api/conversations/${state.current}/compress`,{method:"POST",timeout:150000,body:JSON.stringify({rounds,provider_id:provider.id})}),conversation=state.conversations.find(item=>item.id===state.current);if(conversation)conversation.summary=result.summary;$("#manualCompressStatus").textContent=`已压缩 ${result.rounds} 轮（${result.messages} 条消息），原消息仍可查看。`;$("#manualCompressHint").textContent=`还可继续压缩约 ${result.available_rounds} 轮。`;setTimeout(()=>{$("#manualCompressDialog").hidden=true;},1200);}catch(error){$("#manualCompressStatus").textContent=error.message;}finally{button.disabled=false;button.textContent="开始压缩";}};
...
function updateMemoryRangeLabels(){if($("#memoryImportanceValue"))$("#memoryImportanceValue").textContent=`${Math.round(Number($("#memoryForm").elements.importance.value)*100)}%`;if($("#memoryConfidenceValue"))$("#memoryConfidenceValue").textContent=`${Math.round(Number($("#memoryForm").elements.confidence.value)*100)}%`;}
...
$("#startNowhereAuto").onclick=()=>{$("#startNowhereAuto").hidden=true;$("#stopNowhereAuto").hidden=false;runAutonomyWake({manual:true,mode:"travel",steps:$("#nowhereAutoSteps").value});};$("#stopNowhereAuto").onclick=()=>{stopCurrentGeneration();$("#nowhereAutoStatus").textContent="已停止，旅行记录已保留";$("#startNowhereAuto").hidden=false;$("#stopNowhereAuto").hidden=true;};
renderAutonomy();
