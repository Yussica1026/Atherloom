...
function renderHomeProfile(){const profile=JSON.parse(localStorage.getItem("atherloom:home-profile")||"{}");const name=$("#homeCharacterName");if(!name)return;name.value=profile.name||"";const parts=profile.parts||{hair:"cloud",eyes:"round",face:"soft",mouth:"smile",nose:"dot",clothes:"cloud"};document.querySelectorAll("[data-char-part]").forEach(select=>{select.value=parts[select.dataset.charPart]||select.options[0].value;});const character=$("#roomCharacter");if(character){character.dataset.hair=parts.hair;character.dataset.eyes=parts.eyes;character.dataset.face=parts.face;character.dataset.mouth=parts.mouth;character.dataset.nose=parts.nose;character.dataset.clothes=parts.clothes;}$("#roomPetDot").hidden=!gameState.homestead?.pet;$("#roomRug").textContent=(profile.items||[]).join(" · ")||"⌂";}
