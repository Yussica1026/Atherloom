...
$("#gameRoomForm").onsubmit=async event=>{event.preventDefault();const provider=activeProvider(),content=$("#gameRoomInput").value.trim(),button=event.target.querySelector("button");if(!content)return;if(!provider){$("#gameLibrary").hidden=true;return openSettings("providers");}button.disabled=true;button.textContent="正在回话…";$("#gameRoomInput").disabled=true;try{const gameId=gameState.current,payload=await api(`/api/games/${gameId}/room-chat`,{method:"POST",body:JSON.stringify({provider_id:provider.id,persona_id:state.persona,content}),timeout:45000});$("#gameRoomInput").value="";storeGameState(gameId,payload.state);renderCurrentGame(gameId);}catch(error){alert(error.message);}finally{button.disabled=false;button.textContent="发送";$("#gameRoomInput").disabled=false;$("#gameRoomInput").focus();}};
