...
const cardRoomState = { mode:"poker", hand:["☁ 3","✦ 7","◇ 9","☾ J","✿ A"], played:[], turn:0 };
...
function renderCardRoom(){const s=cardRoomState;$("#cardRoomHand").innerHTML=s.hand.map((card,i)=>`<button class="playing-card ${i===s.turn%Math.max(1,s.hand.length)?"hint":""}" data-card-index="${i}">${card}</button>`).join("");$("#cardRoomPlayed").innerHTML=s.played.map(card=>`<span>${card}</span>`).join("")||"<small>牌桌还很安静</small>";$("#cardRoomStatus").textContent=s.turn%2===0?"轮到你出牌":"当前人格正在思考";$("#cardRoomTurn").textContent=s.turn%2===0?"你的回合":"等待下一轮";$("#cardRoomThought").textContent=s.turn%2?`${activePersonaName()}：这张牌有点意思，我先看看你会怎么走。`:`${activePersonaName()}：我在旁边看着，出牌前可以和我聊聊。`;document.querySelectorAll("[data-card-index]").forEach(button=>button.onclick=()=>{if(s.turn%2)return;const card=s.hand.splice(Number(button.dataset.cardIndex),1)[0];s.played.push(card);s.turn++;setTimeout(()=>{if(s.turn%2&&s.hand.length){s.played.push(s.hand.shift());s.turn++;renderCardRoom();}},500);renderCardRoom();});}
...
function renderPickers() {
  ...
  if(provider) $("#modelPicker").textContent += tokenLabel;
  ...
  if(provider) $("#modelPicker").textContent = `${$("#modelPicker").textContent.replace("⌄","")}${tokenLabel}⌄`;
}
...
function renderLifeRecords(){
  ...
  $("#lifeRecordList").innerHTML=rows.map(item=>`<article class="life-slip kind-${item.kind}"><time>${new Date(item.occurred_at).toLocaleString("zh-CN",{month:"numeric",day:"numeric",hour:"2-digit",minute:"2-digit"})}</time><div><span>${escapeHtml(lifeRecordLabel(item))}</span><strong>${item.kind==="expense"||item.kind==="income"?`${item.kind==="income"?"+":"−"} ¥${Number(item.amount||0).toFixed(2)}`:escapeHtml(item.title||item.note||"已记录")}</strong>${item.note&&item.title?`<small>${escapeHtml(item.note)}</small>`:""}</div><footer>${item.visible_to_ai?"AI 可参考":"仅自己可见"}<button data-delete-life="${item.id}" aria-label="删除这条记录">删除</button></footer></article>`).join("")||`<div class="life-empty"><strong>今天从哪一件小事开始？</strong><span>记下一笔钱、身体的变化，或者刚刚吃过的东西。</span></div>`;
...
const _atherloomOpenGame=openGame;openGame=async function(gameId){if(gameId==="card_room"){gameState.current=gameId;document.querySelectorAll(".mini-game-stage").forEach(stage=>stage.hidden=stage.id!=="cardRoomStage");$("#gameEmpty").hidden=true;$("#aiGameControls").hidden=true;$("#gameRoom").hidden=true;renderCardRoom();return;}return _atherloomOpenGame(gameId);};
