...
function renderHomestead(){
  ...
  $("#homesteadInventory").innerHTML=`<span>宠物粮 × ${current.inventory.pet_food||0}</span><span>肥料 × ${current.inventory.fertilizer||0}</span><span>洗护 × ${current.inventory.soap||0}</span>`;$("#homesteadEvents").innerHTML=[...(current.events||[])].reverse().slice(0,12).map(item=>`<span>${escapeHtml(item.text)}</span>`).join("");
}
