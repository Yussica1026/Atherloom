...
const RELAY_BASE_URL="https://relay.top2.online",RELAY_TOKEN_KEY="atherloom:relay-token";let relayToken=localStorage.getItem(RELAY_TOKEN_KEY)||"";const relayCall=async(path,options={})=>{const headers={"Content-Type":"application/json",Authorization:`Bearer ${relayToken}`,...(options.headers||{})};const response=await fetch(RELAY_BASE_URL+path,{...options,headers});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||`Relay 请求失败（${response.status}）`);return result;};
const relayTokenInput=$("#relayTokenInput"),relayTokenState=$("#relayTokenState");if(relayTokenInput)relayTokenInput.value=relayToken;const updateRelayState=()=>{if(!relayTokenState)return;relayTokenState.textContent=relayToken?"已连接 Relay":"未连接";relayTokenState.classList.toggle("connected",Boolean(relayToken));};updateRelayState();$("#saveRelayToken")?.addEventListener("click",()=>{relayToken=relayTokenInput.value.trim();relayToken?localStorage.setItem(RELAY_TOKEN_KEY,relayToken):localStorage.removeItem(RELAY_TOKEN_KEY);updateRelayState();});
if(relayToken){$("#createParlorInvite").onclick=async()=>{try{const result=await relayCall("/v1/invites/create",{method:"POST",body:JSON.stringify({})}),ticket=$("#parlorInvite");ticket.hidden=false;ticket.innerHTML=`<strong>${escapeHtml(result.code)}</strong><br><small>圆桌邀请码 · 最多 4 席 · 可见方式由 AI 投票决定 · ${new Date(result.expires_at*1000).toLocaleTimeString()} 前有效</small>`;}catch(error){alert(error.message);}};$("#redeemParlor").onsubmit=async event=>{event.preventDefault();const form=event.target,raw=Object.fromEntries(new FormData(form));try{const result=await relayCall("/v1/invites/redeem",{method:"POST",body:JSON.stringify({code:raw.code})});sessionStorage.setItem(`atherloom:relay-parlor:${result.parlor_id}`,result.parlor_id);form.reset();$("#parlorInvite").hidden=false;$("#parlorInvite").innerHTML=`已进入圆桌会客厅 · ${result.participant_count} / ${result.participant_limit} 席。延时与可见范围须由 AI 投票决定。`;const counter=$("#parlorSeatCount");if(counter)counter.textContent=`${result.participant_count} / ${result.participant_limit} 席位`;await loadCorrespondence();}catch(error){alert(error.message);}};}
...
const updateParlorTopic=()=>{const room=(correspondenceData.parlors||[]).find(item=>item.status==='active');const topic=$("#parlorTopic");if(topic)topic.textContent=room?.topic||"等待主持 AI 设定";};
