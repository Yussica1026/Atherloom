...
$("#gameRoomForm").onsubmit=async event=>{event.preventDefault();const provider=activeProvider()||state.providers[0],content=$("#gameRoomInput").value.trim(),button=event.target.querySelector("button");if(!content)return;if(!provider){$("#gameRoomMessages").insertAdjacentHTML("beforeend",'<div class="game-room-message event">还没有可用线路。游戏没有退出；请先到设置里添加一条线路。</div>');return;}state.provider||=provider.id;button.disabled=true;button.textContent="正在回话…";$("#gameRoomInput").disabled=true;try{const gameId=gameState.current,payload=await api(`/api/games/${gameId}/room-chat`,{method:"POST",body:JSON.stringify({provider_id:provider.id,persona_id:state.persona,content}),timeout:45000});$("#gameRoomInput").value="";storeGameState(gameId,payload.state);renderCurrentGame(gameId);}catch(error){$("#gameRoomMessages").insertAdjacentHTML("beforeend",`<div class="game-room-message event">消息没有发出：${escapeHtml(error.message)}。游戏仍保留在当前局面。</div>`);$("#gameRoomMessages").scrollTop=$("#gameRoomMessages").scrollHeight;}finally{button.disabled=false;button.textContent="发送";$("#gameRoomInput").disabled=false;$("#gameRoomInput").focus();}};
