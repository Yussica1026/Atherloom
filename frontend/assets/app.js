...
function renderRoleplayProse(value){return String(value||"").split(/\n+/).filter(Boolean).map(line=>{const match=line.match(/^([^：:\n]{1,24})[：:]\s*([\s\S]*)$/);return match?`<div class="roleplay-speaker"><b>${escapeHtml(match[1])}</b><p>${escapeHtml(match[2])}</p></div>`:`<div class="roleplay-speaker narration"><b>旁白</b><p>${escapeHtml(line)}</p></div>`;}).join("");}
...
async function requestRoleplayOpening(){const story=roleplayState.current;if(!story||roleplayState.busy)return;roleplayState.busy=true;$("#roleplayStatus").textContent="旁白正在写开场…";renderRoleplayStage();try{const opening=await api(`/api/roleplay/stories/${story.id}/opening`,{method:"POST",body:"{}",timeout:150000});if(!story.turns.some(turn=>turn.turn_number===0))story.turns.push(opening);story.state={...story.state,scene:opening.checkpoint.scene,rolling_summary:`开场：${opening.prose}`};$("#roleplayStatus").textContent="开场已经保存";}catch(error){$("#roleplayStatus").textContent=`开场失败：${error.message}`;}finally{roleplayState.busy=false;renderRoleplayStage();}}
...
async function rerollRoleplayTurn(turnNumber){const story=roleplayState.current,turn=story?.turns.find(item=>item.turn_number===turnNumber);if(!turn||roleplayState.busy||!confirm(`重新生成第 ${turnNumber} 回合？这一回合的旧正文会被替换。`))return;roleplayState.busy=true;renderRoleplayStage();$("#roleplayStatus").textContent="正在重写这一回合…";try{const removed=await api(`/api/roleplay/stories/${story.id}/turns/${turnNumber}`,{method:"DELETE"});story.turns=story.turns.filter(item=>item.turn_number!==turnNumber);story.state=removed.state;const replacement=await api(`/api/roleplay/stories/${story.id}/turns`,{method:"POST",body:JSON.stringify({player_input:turn.player_input}),timeout:360000});story.turns.push(replacement);story.state={...story.state,turn_number:replacement.turn_number,scene:replacement.checkpoint.scene,rolling_summary:replacement.checkpoint.rolling_summary||story.state.rolling_summary};$("#roleplayStatus").textContent=`第 ${turnNumber} 回合已经重写`;}catch(error){$("#roleplayStatus").textContent=`重 Roll 失败：${error.message}`;await loadRoleplayStory(story.id);}finally{roleplayState.busy=false;renderRoleplayStage();}}
...
$("#roleplaySetup").onsubmit=async event=>{
  ...
  try{const payload={title:form.elements.title.value.trim(),player_name:form.elements.player_name.value.trim(),premise:form.elements.premise.value.trim(),preset,worldbook_ids,narrator_provider_id:form.elements.narrator_provider_id.value,cast},editing=form.dataset.editing;roleplayState.current=await api(editing?`/api/roleplay/stories/${editing}`:"/api/roleplay/stories",{method:editing?"PUT":"POST",body:JSON.stringify(payload)});if(editing){const index=roleplayState.stories.findIndex(item=>item.id===editing);if(index>=0)roleplayState.stories[index]=roleplayState.current;renderRoleplayStage();$("#roleplayStatus").textContent="故事设置已保存，从下一回合生效";}else{roleplayState.stories.unshift(roleplayState.current);renderRoleplayStage();await requestRoleplayOpening();}}catch(error){alert(error.message);}finally{button.disabled=false;button.textContent="保存并开场";}
};
