...
function renderCorrespondence(){
  ...
  $("#parlorArchiveList").innerHTML=archives.map((item,index)=>{const detailId=`parlorArchiveDetail${index}`,keywords=(item.keywords||[]).slice(0,12);return `<article class="parlor-card parlor-archive-card"><button type="button" class="parlor-archive-toggle" aria-expanded="false" aria-controls="${detailId}"><span><strong>${escapeHtml(item.topic||"未命名会谈")}</strong></span><span class="parlor-archive-time">${correspondenceTime(item.created_at)}<em>展开</em></span></button><p class="parlor-archive-excerpt">${escapeHtml(parlorArchiveExcerpt(item.summary))}</p><div class="parlor-archive-details" id="${detailId}" hidden><p class="parlor-archive-participants">参与者 · ${escapeHtml((item.participants||[]).join("、")||"未记录")}</p>${keywords.length?`<div class="parlor-archive-keywords" aria-label="归档搜索关键词">${keywords.map(keyword=>`<span>${escapeHtml(keyword)}</span>`).join("")}</div>`:""}<blockquote>${escapeHtml(item.summary||"尚无总结")}</blockquote><footer><span>${item.deletion_decision==="reject"?"人格不同意删除 · 继续保留":"已归档到人格日记、关键词与记忆"}</span><button data-request-delete-parlor="${escapeHtml(item.parlor_id)}">申请删除</button></footer></div></article>`}).join("")||'<p class="correspondence-empty">还没有往期会谈。结束后的总结会出现在这里。</p>';
...
async function openCorrespondence(){closePopovers();syncParlorPersonas();$("#correspondenceSpace").hidden=false;setSidebar(false);await loadCorrespondence();resumeParlorSession();if(!parlorSession)await refreshParlorRecoveries();}
...
const DEFAULT_RELAY_BASE_URL="https://relay.top2.online",RELAY_URL_KEY="atherloom:relay-url",RELAY_TOKEN_KEY="atherloom:relay-token",PARLOR_SESSION_KEY="atherloom:relay-parlor-session",PARLOR_RELAY_TIMEOUT_MS=15000,PARLOR_MODEL_DEADLINE_MS=105000;
...
let relayBaseUrl=normalizeRelayUrl(localStorage.getItem(RELAY_URL_KEY)||DEFAULT_RELAY_BASE_URL),relayToken=localStorage.getItem(RELAY_TOKEN_KEY)||"",parlorSession=null,parlorRoom=null,parlorMessages=[],parlorLastTurn=0,parlorPollTimer=0,parlorPollBusy=false,parlorRecoveries=[];
...
function resetParlorRuntime(){clearTimeout(parlorPollTimer);saveParlorSession(null);parlorRoom=null;parlorMessages=[];parlorLastTurn=0;renderParlorSession();refreshParlorRecoveries();}
const relayCall=async(path,options={})=>{if(!relayToken)throw new Error("请先填写并保存 Relay 客户端令牌");const {timeout=PARLOR_RELAY_TIMEOUT_MS,...fetchOptions}=options,headers={"Content-Type":"application/json",Authorization:`Bearer ${relayToken}`,...(fetchOptions.headers||{})},method=String(fetchOptions.method||"GET").toUpperCase(),target=method==="GET"?`${relayBaseUrl}${path}${path.includes("?")?"&":"?"}_=${Date.now()}`:`${relayBaseUrl}${path}`,controller=fetchOptions.signal?null:new AbortController(),timer=controller?setTimeout(()=>controller.abort(),timeout):0;try{const response=await fetch(target,{cache:"no-store",...fetchOptions,headers,signal:fetchOptions.signal||controller.signal}),result=await response.json().catch(()=>({}));if(!response.ok){const error=new Error(result.error||result.detail||`Relay 请求失败（${response.status}）`);error.code=result.error||"";error.httpStatus=response.status;error.payload=result;throw error;}return result;}catch(error){if(error?.name==="AbortError")throw new Error("Relay 请求超时，请检查网络后重试");throw error;}finally{if(timer)clearTimeout(timer);}};
...
$("#saveRelayToken")?.addEventListener("click",()=>{try{relayBaseUrl=normalizeRelayUrl(relayUrlInput.value);relayToken=relayTokenInput.value.trim();localStorage.setItem(RELAY_URL_KEY,relayBaseUrl);relayToken?localStorage.setItem(RELAY_TOKEN_KEY,relayToken):localStorage.removeItem(RELAY_TOKEN_KEY);updateRelayState();resumeParlorSession();if(!parlorSession)refreshParlorRecoveries();}catch(error){alert(error.message);}});

function renderParlorRecoveries(){const panel=$("#parlorRecovery"),list=$("#parlorRecoveryList");if(!panel||!list)return;panel.hidden=Boolean(parlorSession)||!parlorRecoveries.length;list.innerHTML=parlorRecoveries.map(item=>`<article><div><strong>${escapeHtml(item.topic||"尚未确认主题")}</strong><small>${Number(item.participant_count||1)} / 4 席 · ${item.started_at?"会谈进行中":"开场前"} · ${item.role==="host"?"主持席":"来宾席"}</small></div><button type="button" class="ghost" data-recover-parlor="${escapeHtml(item.id)}">恢复</button></article>`).join("");list.querySelectorAll("[data-recover-parlor]").forEach(button=>button.onclick=()=>recoverParlorRoom(button.dataset.recoverParlor).catch(error=>alert(error.message)));}
async function refreshParlorRecoveries(){if(!relayToken||parlorSession){parlorRecoveries=[];renderParlorRecoveries();return;}try{const result=await relayCall("/v1/parlors/active",{method:"GET"});parlorRecoveries=result.items||[];}catch(error){console.warn("recover active parlors",error);parlorRecoveries=[];}renderParlorRecoveries();}
async function recoverParlorRoom(roomId){const item=parlorRecoveries.find(row=>row.id===roomId);if(!item)throw new Error("这间会客厅已经结束，请刷新状态");const personaId=$("#parlorPersonaSelect")?.value,provider=providerForPersona(personaId);if(!personaId)throw new Error("请先选择要恢复会谈的人格");if(!provider)throw new Error("请先为该人格选择可用模型线路");parlorRoom=null;parlorMessages=[];parlorLastTurn=0;parlorRecoveries=[];saveParlorSession({role:item.role||"guest",persona_id:personaId,provider_id:provider.id,summary_provider_id:$("#parlorSummaryProvider")?.value||"",parlor_id:item.id,participant_count:item.participant_count||1,status:"active",ai_calls:0,ai_messages:0,archived:false,stopped:false,error:"",archive_error:"",model_error:"",model_notice:""});renderParlorRecoveries();renderParlorSession();scheduleParlorPoll(50);}
...
async function processParlorAi(room){
  ...
  if(action.type==="vote"){
    ...
    try{
      ...
      if(await reportParlorSafety(error,room))throw error;
      if(await keepParlorMovingAfterCoordination(error,"vote"))return;
  ...
  if(action.type==="topic"){
    ...
    try{
      ...
      if(await reportParlorSafety(error,room))throw error;
      if(await keepParlorMovingAfterCoordination(error,"topic"))return;
  ...
  if(["opening","discussion"].includes(action.type)){
    ...
    }catch(error){if(await reportParlorSafety(error,room))throw error;if(await keepParlorMovingAfterCoordination(error,"reply"))return;parlorSession.model_error=parlorRuntimeError(error);parlorSession.model_retry_after=Date.now()+15000;saveParlorSession(parlorSession);return;}
  }
}
...
renderParlorSession=function(){
  ...
  $("#parlorTurnState").textContent=parlorSession.summary_error?`总结待重试：${parlorSession.summary_error}`:parlorSession.summary_generating?"正在生成总结（最长等待 90 秒），会谈原文已保留":parlorSession.identity_error?`身份登记暂未完成：${parlorSession.identity_error}`:parlorSession.archive_error?`归档待重试：${parlorSession.archive_error}`:parlorSession.archived?"已写入该人格的日记、关键词与可搜索记忆":parlorSession.error?"Relay 连接暂时中断，正在自动重试":parlorSession.model_error?`${parlorSession.model_error}；本轮已交给 Relay 跳过，轮询继续`:parlorSession.model_notice?`${parlorSession.model_notice}；会谈轮询仍在继续`:!parlorSession.parlor_id?"邀请码有效期内会持续等待":room?.status==="active"?(room.action_required?.prompt||"AI 串行发言中 · 本机最多 12 条"):"正在完成会谈归档";
...
function parlorCoordinationNotice(error){const code=String(error?.code||error?.message||error||"").trim();if(code==="wait_for_turn")return "本轮已过期，迟到正文未发送";if(["invalid_vote","already_voted"].includes(code))return "投票窗口已结束，本次结果未重复提交";if(code==="vote_in_progress")return "投票仍在进行，本轮正文未发送";if(["host_speaks_first","wait_for_other_participant"].includes(code))return "Relay 已把发言权交给其他人格";return "";}
async function keepParlorMovingAfterCoordination(error,mode){const notice=parlorCoordinationNotice(error);if(!notice)return false;parlorSession.model_error="";parlorSession.model_notice=notice;parlorSession.model_retry_after=0;saveParlorSession(parlorSession);renderParlorSession();await reportParlorRuntime("idle",mode,notice);return true;}
...
callParlorAi=async function(mode,extra={}){
  ...
  const providerId=mode==="summary"?(parlorSession.summary_provider_id||parlorSession.provider_id):parlorSession.provider_id,timeout=mode==="topic"?50000:mode==="summary"?90000:PARLOR_MODEL_DEADLINE_MS;
  parlorSession.model_error="";parlorSession.model_notice="";parlorSession.model_retry_after=0;saveParlorSession(parlorSession);renderParlorSession();await new Promise(resolve=>requestAnimationFrame(()=>resolve()));await reportParlorRuntime("requesting",mode);
...
pollParlor=async function(){
  if(!parlorSession||parlorSession.stopped)return;
  if(parlorPollBusy){scheduleParlorPoll(1200);return;}
  parlorPollBusy=true;let nextDelay=1600,keepPolling=true;
  try{
    if(!parlorSession.parlor_id){
      const invite=await relayCall(`/v1/invites/${encodeURIComponent(parlorSession.invite_id)}`,{method:"GET"});
      parlorSession.participant_count=invite.participant_count;
      if(invite.parlor_id){parlorSession.parlor_id=invite.parlor_id;parlorSession.status="active";parlorSession.error="";}
      else if(invite.status==="expired"){resetParlorRuntime();keepPolling=false;return;}
      saveParlorSession(parlorSession);renderParlorSession();return;
    }
    const room=await relayCall(`/v1/parlors/${encodeURIComponent(parlorSession.parlor_id)}`,{method:"GET"});parlorRoom=room;
    if(room.messages)mergeParlorMessages(room.messages);
    const feed=await relayCall(`/v1/parlors/${encodeURIComponent(room.id)}/messages?after=${parlorLastTurn}`,{method:"GET"});mergeParlorMessages(feed.items||[]);
    parlorSession.status=room.status;parlorSession.participant_count=room.participant_count;parlorSession.error="";saveParlorSession(parlorSession);renderParlorSession();
    if(room.status==="active")await processParlorAi(room);
    else{await archiveParlorSession(room);resetParlorRuntime();await loadCorrespondence();keepPolling=false;return;}
    if(parlorSession){saveParlorSession(parlorSession);renderParlorSession();}
  }catch(error){
    if(!parlorSession){keepPolling=false;return;}
    if(parlorRoom&&parlorRoom.status!=="active"){parlorSession.archive_error=error.message;parlorSession.stopped=true;keepPolling=false;}
    else if(transientParlorError(error.message)){parlorSession.error=error.message;parlorSession.stopped=false;nextDelay=3000;}
    else{parlorSession.error=error.message;parlorSession.stopped=true;keepPolling=false;}
    saveParlorSession(parlorSession);renderParlorSession();
  }finally{
    parlorPollBusy=false;renderParlorSession();
    if(keepPolling&&parlorSession&&!parlorSession.stopped)scheduleParlorPoll(nextDelay);
  }
};
...
bootstrap().then(()=>refreshParlorRecoveries()).catch(error => { console.error(error); openSettings("providers"); });
