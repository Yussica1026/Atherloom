...
$("#roleplayBackstageForm").onsubmit=async event=>{event.preventDefault();const story=roleplayState.current,input=event.target.querySelector("textarea"),button=event.target.querySelector("button"),content=input.value.trim();if(!story||!content)return;button.disabled=true;$("#roleplayBackstageStatus").textContent="AI 本体正在看当前剧情…";try{const result=await api(`/api/roleplay/stories/${story.id}/backstage`,{method:"POST",body:JSON.stringify({content}),timeout:150000});story.backstage=result.backstage;input.value="";renderRoleplayBackstage();$("#roleplayBackstageStatus").textContent="这段讨论只留在幕后，不进入正文";}catch(error){$("#roleplayBackstageStatus").textContent=`讨论失败：${error.message}`;}finally{button.disabled=false;}};
