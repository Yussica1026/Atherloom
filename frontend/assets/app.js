...
async function updateHistoryState(id, action) {
  ...
  if(action==="delete"){
    ...
    await api(`/api/conversations/${id}`,{method:"DELETE"});state.conversations=state.conversations.filter(item=>item.id!==id);state.message_cache.delete(id);
    if(state.current===id){state.current=null;state.messages=[];const next=state.conversations.find(item=>(item.persona_id||null)===(state.persona||null));if(next)await openConversation(next.id);else await newConversation();}
...
$("#personaPicker").onclick = e => { e.stopPropagation(); showPopover(e.currentTarget, $("#personaPopover"), `<button data-value="">默认人格</button>` + state.personas.map(p => `<button data-value="${p.id}">${escapeHtml(p.name)}</button>`).join(""), async id => { state.persona = id || null;if(state.persona)localStorage.setItem("atherloom:last-persona",state.persona);else localStorage.removeItem("atherloom:last-persona");state.current=null;state.messages=[];state.memories=await api(`/api/memories?persona_key=${encodeURIComponent(memoryPersonaKey())}`);const persona=activePersona(),startup=startupConversationPlan(persona,state.conversations);if(startup.mode==="new")await newConversation();else if(startup.conversationId)await openConversation(startup.conversationId);else await newConversation();renderSettings();renderPickers(); }); };
