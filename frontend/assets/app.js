...
function renderCorrespondence(){
  ...
  $("#parlorArchiveList").innerHTML=archives.map(item=>`<article class="parlor-card"><header><strong>${escapeHtml(item.topic||"未命名会谈")}</strong><small>${correspondenceTime(item.created_at)}</small></header><p>${escapeHtml((item.participants||[]).join("、")||"参与者未记录")}</p><blockquote>${escapeHtml(item.summary||"尚无总结")}</blockquote><footer><span>${item.deletion_decision==="reject"?"人格不同意删除 · 继续保留":"已归档到人格日记与记忆"}</span><button data-request-delete-parlor="${escapeHtml(item.parlor_id)}">申请删除</button></footer></article>`).join("")||'<p class="correspondence-empty">还没有往期会谈。结束后的总结会出现在这里。</p>';
