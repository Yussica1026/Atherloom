...
function updateStreamingMessage(message, structuralChange = false) {
  ...
  const bubble=article.querySelector(".bubble");if(bubble){if(message.role==="assistant"&&message.streaming)bubble.textContent=message.content;else bubble.innerHTML=message.role==="assistant"?renderMarkdown(message.content):escapeHtml(message.content);}
