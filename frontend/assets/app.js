...
$("#stickyReplyForm").onsubmit=async event=>{event.preventDefault();const note=stickyQueue[stickyIndex],content=event.target.elements.content.value.trim();if(!note||!content)return;await api(`/api/board/${encodeURIComponent(note.persona_id||"__default__")}`,{method:"POST",body:JSON.stringify({content,author:"user",visible_to_user:true,visible_to_ai:true,reply_to:note.id})});event.target.hidden=true;event.target.reset();$("#stickyCounter").textContent="回复已送到这个人格的独立留言板";};
