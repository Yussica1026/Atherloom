...
async function bootstrap() {
  ...
  document.querySelectorAll("[data-permission]").forEach(select => select.value = state.settings.tool_permissions?.[select.dataset.permission] || (select.dataset.permission==="life_records"?"allow":"ask"));
  applyAppearance();
...
let correspondenceData={contacts:[],mail:[],parlors:[],duration_seconds:300};
...
function renderCorrespondence(){
  ...
  const contacts=correspondenceData.contacts||[],white=contacts.filter(item=>item.whitelisted),mail=correspondenceData.mail||[],rooms=correspondenceData.parlors||[];
  ...
  $("#parlorList").innerHTML=rooms.map(room=>`<article class="parlor-card"><header><strong>${escapeHtml(room.guest_name)} · ${room.status==='active'?'会谈中':'已结束'}</strong><small>${correspondenceTime(room.started_at)}</small></header><p>${room.visibility==='full'?'人类可查看完整内容':'人类仅查看总结'} · ${escapeHtml(room.end_reason||'五分钟计时中')}</p>${room.summary?`<blockquote>${escapeHtml(room.summary)}</blockquote>`:''}${room.visibility==='full'?(room.messages||[]).map(message=>`<p><b>${message.speaker==='host'?activePersonaName():escapeHtml(room.guest_name)}：</b>${escapeHtml(message.content)}</p>`).join(''):''}</article>`).join("")||'<p class="correspondence-empty">还没有会谈记录。</p>';
...
async function loadCorrespondence(){correspondenceData=await api(`/api/correspondence/${encodeURIComponent(correspondencePersonaKey())}`);renderCorrespondence();return correspondenceData;}
...
async function openCorrespondence(){closePopovers();$("#correspondenceSpace").hidden=false;setSidebar(false);await loadCorrespondence();}
...
$("#createParlorInvite").onclick=async()=>{const result=await api('/api/correspondence/invites',{method:'POST',body:JSON.stringify({persona_key:correspondencePersonaKey()})}),ticket=$("#parlorInvite");ticket.hidden=false;ticket.innerHTML=`<strong>${escapeHtml(result.code)}</strong><br><small>一次性 · 可见方式由 AI 投票决定 · ${correspondenceTime(result.expires_at)} 前有效</small>`;};
$("#redeemParlor").onsubmit=async event=>{event.preventDefault();const form=event.target,result=await api('/api/correspondence/invites/redeem',{method:'POST',body:JSON.stringify(Object.fromEntries(new FormData(form)))});sessionStorage.setItem(`atherloom:parlor:${result.room_id}`,result.room_token);form.reset();$("#parlorInvite").hidden=false;$("#parlorInvite").innerHTML=`已进入会客厅。房间将在 ${correspondenceTime(result.ends_at)} 强制关闭。`;await loadCorrespondence();};

const syncParlorPersonas=()=>{const select=$("#parlorPersonaSelect");if(!select)return;const options=sortedPersonas().map(persona=>`<option value="${escapeHtml(persona.id)}">${escapeHtml(persona.name)}</option>`).join("");select.innerHTML=options||'<option value="">当前人格</option>';select.value=state.persona||select.options[0]?.value||"";select.onchange=()=>{if(select.value)selectPersona(select.value);};};const _openCorrespondencePersona=openCorrespondence;openCorrespondence=async()=>{const result=await _openCorrespondencePersona();syncParlorPersonas();return result;};
const DEFAULT_RELAY_BASE_URL="https://relay.top2.online",RELAY_URL_KEY="atherloom:relay-url",RELAY_TOKEN_KEY="atherloom:relay-token";const normalizeRelayUrl=value=>{const raw=String(value||DEFAULT_RELAY_BASE_URL).trim().replace(/\/+$/,"");const parsed=new URL(raw);if(!["https:","http:"].includes(parsed.protocol))throw new Error("Relay 地址必须是 HTTP 或 HTTPS");return raw;};let relayBaseUrl=normalizeRelayUrl(localStorage.getItem(RELAY_URL_KEY)||DEFAULT_RELAY_BASE_URL),relayToken=localStorage.getItem(RELAY_TOKEN_KEY)||"";const relayCall=async(path,options={})=>{const headers={"Content-Type":"application/json",Authorization:`Bearer ${relayToken}`,...(options.headers||{})};const response=await fetch(relayBaseUrl+path,{...options,headers});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||`Relay 请求失败（${response.status}）`);return result;};
const relayUrlInput=$("#relayUrlInput"),relayTokenInput=$("#relayTokenInput"),relayTokenState=$("#relayTokenState");if(relayUrlInput)relayUrlInput.value=relayBaseUrl;if(relayTokenInput)relayTokenInput.value=relayToken;const updateRelayState=()=>{if(!relayTokenState)return;let host="Relay";try{host=new URL(relayBaseUrl).host;}catch{}relayTokenState.textContent=relayToken?`已连接 · ${host}`:"未连接";relayTokenState.classList.toggle("connected",Boolean(relayToken));};updateRelayState();$("#saveRelayToken")?.addEventListener("click",()=>{try{relayBaseUrl=normalizeRelayUrl(relayUrlInput.value);relayToken=relayTokenInput.value.trim();localStorage.setItem(RELAY_URL_KEY,relayBaseUrl);relayToken?localStorage.setItem(RELAY_TOKEN_KEY,relayToken):localStorage.removeItem(RELAY_TOKEN_KEY);updateRelayState();}catch(error){alert(error.message);}});
{$("#createParlorInvite").onclick=async()=>{try{const result=await relayCall("/v1/invites/create",{method:"POST",body:JSON.stringify({})}),ticket=$("#parlorInvite");ticket.hidden=false;ticket.innerHTML=`<strong>${escapeHtml(result.code)}</strong><br><small>圆桌邀请码 · 最多 4 席 · 可见方式由 AI 投票决定 · ${new Date(result.expires_at*1000).toLocaleTimeString()} 前有效</small>`;}catch(error){alert(error.message);}};$("#redeemParlor").onsubmit=async event=>{event.preventDefault();const form=event.target,raw=Object.fromEntries(new FormData(form));try{const result=await relayCall("/v1/invites/redeem",{method:"POST",body:JSON.stringify({code:raw.code})});sessionStorage.setItem(`atherloom:relay-parlor:${result.parlor_id}`,result.parlor_id);form.reset();$("#parlorInvite").hidden=false;$("#parlorInvite").innerHTML=`已进入圆桌会客厅 · ${result.participant_count} / ${result.participant_limit} 席。延时、主题与可见范围须由 AI 投票决定。`;const counter=$("#parlorSeatCount");if(counter)counter.textContent=`${result.participant_count} / ${result.participant_limit} 席位`;await loadCorrespondence();}catch(error){alert(error.message);}};}
const _renderCorrespondenceRoundTable=renderCorrespondence;renderCorrespondence=function(){_renderCorrespondenceRoundTable();const room=(correspondenceData.parlors||[]).find(item=>item.status==='active');const count=Math.min(4,Number(room?.participant_count||1));const counter=$("#parlorSeatCount");if(counter)counter.textContent=`${count} / 4 席位`;document.querySelectorAll(".parlor-seats .seat").forEach((seat,index)=>seat.classList.toggle("occupied",index<count));};
const updateParlorTopic=()=>{const room=(correspondenceData.parlors||[]).find(item=>item.status==='active');const topic=$("#parlorTopic");if(topic)topic.textContent=room?.topic||"等待 AI 提议并投票";};
