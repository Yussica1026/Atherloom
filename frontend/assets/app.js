...
$("#personaForm").onsubmit = async e => { e.preventDefault(); const data=Object.fromEntries(new FormData(e.target)),editing=e.target.dataset.editing,saved=await api(editing?`/api/personas/${editing}`:"/api/personas",{method:editing?"PUT":"POST",body:JSON.stringify(data)});if(editing)Object.assign(state.personas.find(item=>item.id===editing),saved);else state.personas.push(saved);state.persona||=saved.id;e.target.reset();delete e.target.dataset.editing;$("#savePersona").textContent="保存人格";$("#cancelPersonaEdit").hidden=true;renderSettings();renderPickers();};
$("#cancelPersonaEdit").onclick=()=>{const form=$("#personaForm");form.reset();delete form.dataset.editing;$("#savePersona").textContent="保存人格";$("#cancelPersonaEdit").hidden=true;};
