...
async function sendRoleplayBackstage(content){const story=roleplayState.current,form=$("#roleplayBackstageForm"),button=form.querySelector("button");if(!story||!content||button.disabled)return;if(!story.premise?.trim()){$("#roleplayBackstageStatus").textContent="核心设定还没保存：请先打开“故事设置”补充，避免 AI 胡编";return;}const saved=[...(story.backstage||[])];story.backstage=[...saved,{role:"user",content},{role:"assistant",content:"正在结合当前故事思考…",pending:true}];renderRoleplayBackstage();button.disabled=true;button.textContent="回应中";$("#roleplayBackstageStatus").textContent=`${activePersonaName()}正在重读当前剧情…`;try{const result=await api(`/api/roleplay/stories/${story.id}/backstage`,{method:"POST",body:JSON.stringify({content,provider_id:activeProvider()?.id,persona_id:state.persona}),timeout:150000});story.backstage=result.backstage;renderRoleplayBackstage();$("#roleplayBackstageStatus").textContent="这段讨论只留在幕后，不进入正文";}catch(error){story.backstage=saved;renderRoleplayBackstage();$("#roleplayBackstageStatus").textContent=`讨论失败：${error.message}`;}finally{button.disabled=false;button.textContent="发送";}}
...
$("#clearRoleplayBackstage").onclick=async()=>{const story=roleplayState.current;if(!story||!story.backstage?.length||!confirm("清空这个故事的全部幕后讨论？小说正文不会改变。"))return;const result=await api(`/api/roleplay/stories/${story.id}/backstage/clear`,{method:"DELETE"});story.backstage=result.backstage;renderRoleplayBackstage();$("#roleplayBackstageStatus").textContent="幕后讨论已经清空";};
