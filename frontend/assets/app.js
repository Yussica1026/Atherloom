...
function openConversationSwitcher(event) {
  ...
  const items = `<button data-value="__new__"><strong>＋ 新对话</strong></button><p class="conversation-longpress-hint">长按任意对话约半秒，显示删除按钮</p>${recent.map(item => `<div class="conversation-switch-row"><button data-value="${item.id}" class="${item.id === state.current ? "active" : ""}"><strong><span>${escapeHtml(item.title)}</span>${generationDot(item.id)}</strong><small>${item.id === state.current ? "当前对话 · 长按显示删除" : `${new Date(item.updated_at || item.created_at).toLocaleString("zh-CN")} · 长按显示删除`}</small></button><button type="button" class="conversation-switch-delete" data-delete-switch="${item.id}" aria-label="删除 ${escapeHtml(item.title)}">删除</button></div>`).join("")}<button data-value="__rename__" ${state.current ? "" : "disabled"}>重命名当前对话</button><button data-value="__clear__" class="conversation-clear-all" ${scopedCount ? "" : "disabled"}>清空当前人格全部对话（${scopedCount}）</button>`;
...
function bindConversationLongPress(popover){
  popover.querySelectorAll('.conversation-switch-row button[data-value]').forEach(button=>{let timer=null,startX=0,startY=0;const cancel=()=>{clearTimeout(timer);timer=null;button.classList.remove("longpress-armed");};button.addEventListener("contextmenu",event=>event.preventDefault());button.addEventListener("pointerdown",event=>{if(event.button!==undefined&&event.button!==0)return;cancel();startX=event.clientX;startY=event.clientY;button.classList.add("longpress-armed");timer=setTimeout(()=>{timer=null;button.dataset.longPressFired="1";button.classList.remove("longpress-armed");popover.querySelectorAll(".conversation-switch-row.delete-revealed").forEach(row=>row.classList.remove("delete-revealed"));button.closest(".conversation-switch-row")?.classList.add("delete-revealed");navigator.vibrate?.(35);},480);});button.addEventListener("pointermove",event=>{if(timer&&Math.hypot(event.clientX-startX,event.clientY-startY)>12)cancel();});button.addEventListener("pointerup",cancel);button.addEventListener("pointercancel",()=>{if(timer)cancel();});button.addEventListener("click",event=>{if(button.dataset.longPressFired==="1"){event.preventDefault();event.stopImmediatePropagation();delete button.dataset.longPressFired;}},true);});
}
