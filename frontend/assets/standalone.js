(() => {
  ...
  const webChat = async request => {
    ...
    const payload={model:provider.model,max_tokens:Number(request.max_tokens||provider.max_tokens||4096),temperature:Number(request.temperature??provider.temperature??.7),top_p:Number(request.top_p??provider.top_p??1),messages:request.preformatted?request.messages:formatMessages(request.messages,provider.protocol)};if(anthropic&&request.system)payload.system=request.system;
  ...
  const builtinTools=()=>{const permissions=settings().tool_permissions||{},tools=[];if(permissions.web_search==="allow")tools.push({name:"atherloom_web_search",description:"搜索公开互联网，返回相关网页标题、链接与摘要。",input_schema:{type:"object",properties:{query:{type:"string"},max_results:{type:"integer",minimum:1,maximum:8}},required:["query"]}});if(permissions.memory_read==="allow")tools.push({name:"atherloom_memory_search",description:"检索本地长期记忆，返回准确 memory_id。",input_schema:{type:"object",properties:{query:{type:"string"}}}});if(permissions.memory_write==="allow"){tools.push({name:"atherloom_memory_create",description:"新增用户明确要求长期保存的本地记忆。",input_schema:{type:"object",properties:{title:{type:"string"},content:{type:"string"},kind:{type:"string"}},required:["title","content"]}});tools.push({name:"atherloom_memory_update",description:"按 memory_id 精确更新已有记忆；先搜索再更新。",input_schema:{type:"object",properties:{memory_id:{type:"string"},title:{type:"string"},content:{type:"string"},kind:{type:"string"}},required:["memory_id"]}});}return tools;};
  const runBuiltinTool=async call=>{const args=call.arguments||{},name=call.name,now=new Date().toISOString();if(name==="atherloom_web_search"){const query=String(args.query||"").trim();if(!query)throw new Error("搜索关键词不能为空");if(native?.webSearch)return nativeResult("webSearch",{query,max_results:Math.max(1,Math.min(Number(args.max_results||5),8))});const response=await originalFetch(`https://api.duckduckgo.com/?q=${encodeURIComponent(query)}&format=json&no_html=1&skip_disambig=1`),data=await response.json(),rows=[];if(data.AbstractURL)rows.push({title:data.Heading||query,url:data.AbstractURL,snippet:data.AbstractText||""});for(const item of data.RelatedTopics||[]){const row=item.Topics?.[0]||item;if(row.FirstURL&&rows.length<Number(args.max_results||5))rows.push({title:String(row.Text||"").split(" - ")[0],url:row.FirstURL,snippet:row.Text||""});}return {query,results:rows};}if(name==="atherloom_memory_search")return {memories:relevantMemories(String(args.query||"")).map(item=>({memory_id:item.id,title:item.title,content:item.content,kind:item.kind,updated_at:item.updated_at}))};if(name==="atherloom_memory_create"){if(!String(args.title||"").trim()||!String(args.content||"").trim())throw new Error("记忆标题和内容不能为空");const item={id:uid(),title:String(args.title).trim(),content:String(args.content).trim(),kind:args.kind||"fact",starred:false,archived:false,created_at:now,updated_at:now};write("memories",[item,...read("memories",[])]);return {created:true,memory_id:item.id,title:item.title};}if(name==="atherloom_memory_update"){const rows=read("memories",[]),item=rows.find(row=>row.id===args.memory_id&&!row.trash);if(!item)throw new Error("找不到该 memory_id；请先搜索记忆");for(const key of ["title","content","kind"])if(args[key]!==undefined)item[key]=String(args[key]).trim();item.updated_at=now;write("memories",rows);return {updated:true,memory_id:item.id,title:item.title};}throw new Error(`未知工具：${name}`);};
  ...
  const discoverRemoteMcp=async server=>{if(server.transport==="stdio")throw new Error("Android 不支持本机 stdio MCP");let result=await mcpPost(server,"initialize",{protocolVersion:"2025-03-26",capabilities:{},clientInfo:{name:"Atherloom Android",version:"0.5.21"}}),session=result.session_id||"";await mcpPost(server,"notifications/initialized",undefined,null,session);result=await mcpPost(server,"tools/list",{},2,session);return result.response?.result?.tools||[];};
  const callRemoteMcp=async(server,name,args)=>{let result=await mcpPost(server,"initialize",{protocolVersion:"2025-03-26",capabilities:{},clientInfo:{name:"Atherloom Android",version:"0.5.21"}}),session=result.session_id||"";await mcpPost(server,"notifications/initialized",undefined,null,session);result=await mcpPost(server,"tools/call",{name,arguments:args||{}},3,session);return result.response?.result||{};};
  ...
  const invokeChatTool=async(call,bindings)=>{if(call.name.startsWith("atherloom_"))return runBuiltinTool(call);const binding=bindings[call.name];if(!binding)throw new Error(`未找到工具 ${call.name}`);return callRemoteMcp(binding.server,binding.original,call.arguments||{});};
  const parseDsmlToolCalls=content=>{
    const text=String(content||""),marker="[|｜]\\s*DSML\\s*[|｜]",calls=[];
  ...
  const inferAiGameAction=(gameId,text)=>{const source=String(text||"").toLowerCase(),aliases={quiet_fishing:[["sell_all",["sell_all","出售","卖掉"]],["buy_bait",["buy_bait","买鱼饵","购买鱼饵"]],["cast",["cast","抛竿","钓鱼"]]],claw_machine:[["move_left",["move_left","向左","左移"]],["move_right",["move_right","向右","右移"]],["grab",["grab","抓取","下爪","抓娃娃"]]],cloud_slots:[["spin",["spin","转动","拉杆","老虎机"]]]};const found=(aliases[gameId]||[]).find(([,words])=>words.some(word=>source.includes(word)));return found?{action:found[0],amount:1}:null;};
  const aiGameComment=(choice,text,structured)=>String(choice?.comment||(!structured?text:"")||"").replace(/```(?:json)?|```/gi,"").trim().slice(0,160);
  ...
  window.fetch = async (input, options = {}) => {
    ...
    if (url.pathname === "/api/providers" && method === "POST") {
      ...
      const source=read("providers",[]).find(item=>item.id===body.source_provider_id),item={...body,api_key:body.api_key||source?.api_key||"",id:uid(),has_api_key:!!(body.api_key||source?.api_key)};write("providers",[...read("providers",[]),item]);return json(publicProvider(item));
    }
    ...
    if(url.pathname==="/api/chat" && method==="POST") {
      ...
      try {
        ...
        const builtin=builtinTools(),remote=await remoteMcpCatalog(personaConfig),tools=[...builtin,...remote.tools];
        if(tools.length){
          ...
          if(detectedCalls.length){
            ...
            const final=native?await nativeChat(request):await webChat(request),reasoning=[probe.reasoning,final.reasoning].filter(Boolean).join("\n\n"),output=persistStreamEvent({reasoning_delta:reasoning,delta:final.content||""});
            return ndjson([...output,...persistStreamEvent({done:true})]);
          }
          const output=persistStreamEvent({reasoning_delta:probe.reasoning||"",delta:probe.content||""});
          return ndjson([...output,...persistStreamEvent({done:true})]);
        }
