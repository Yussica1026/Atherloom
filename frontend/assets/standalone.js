(() => {
  ...
  window.fetch = async (input, options = {}) => {
    ...
    if (url.pathname === "/api/bootstrap") return json({ providers:providers(),personas:read("personas",[]),worldbooks:read("worldbooks",[]),mcp_servers:read("mcp_servers",[]),conversations:read("conversations",[]),settings:settings() });
    if(url.pathname==="/api/mcp-audit"&&method==="GET")return json(read("mcp_audit",[]));
    if(url.pathname==="/api/mcp-servers"&&method==="POST"){const item={...body,id:uid(),has_token:!!body.token,token:undefined,tools:[],tool_policies:body.tool_policies||{},last_status:"",created_at:new Date().toISOString(),updated_at:new Date().toISOString()};write("mcp_servers",[item,...read("mcp_servers",[])]);return json(item);}
    if(url.pathname==="/api/mcp-servers/test"&&method==="POST")return json({detail:"Android 不能直接测试本机 stdio；远程 MCP 请在电脑端后端测试"},422);
    const mcpMatch=url.pathname.match(/^\/api\/mcp-servers\/([^/]+)$/);if(mcpMatch&&method==="PUT"){const rows=read("mcp_servers",[]),index=rows.findIndex(item=>item.id===mcpMatch[1]);if(index<0)return json({detail:"MCP 服务不存在"},404);rows[index]={...rows[index],...body,token:undefined,has_token:rows[index].has_token||!!body.token,updated_at:new Date().toISOString()};write("mcp_servers",rows);return json(rows[index]);}if(mcpMatch&&method==="DELETE"){write("mcp_servers",read("mcp_servers",[]).filter(item=>item.id!==mcpMatch[1]));return json({ok:true});}
    if(url.pathname.match(/^\/api\/mcp-servers\/[^/]+\/refresh$/)&&method==="POST")return json({detail:"请在电脑端后端刷新 MCP 工具；Android 仅保存远程连接配置"},422);
    ...
    if(url.pathname==="/api/chat" && method==="POST") {
      ...
      try {
        ...
        if(tools.length){
          ...
          if(probe.tool_calls?.length){
            const calls=probe.tool_calls.slice(0,8),results=[];
            for(const call of calls){try{results.push(await runBuiltinTool(call));const audit=read("mcp_audit",[]);audit.unshift({id:uid(),server_id:"__builtin__",server_name:"Atherloom 内置工具",tool_name:call.name,status:"success",detail:"",created_at:new Date().toISOString()});write("mcp_audit",audit.slice(0,100));}catch(error){results.push({error:error.message});}}
