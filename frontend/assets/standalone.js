  ...
  window.fetch = async (input, options = {}) => {
    ...
    if(aiTurn&&method==="POST"){
      ...
      for(let turn=0;turn<(body.turns||1);turn++){const key=`game:${gameId}`,fallback=gameId==="quiet_fishing"?defaultGame():gameId==="claw_machine"?defaultClaw():defaultSlots(),current=read(key,fallback),persona=read("personas",[]).find(item=>item.id===body.persona_id),instruction=`${persona?.prompt?persona.prompt+"\n\n":""}你正在 Atherloom 中玩游戏 ${gameId}。\n当前状态：${JSON.stringify(current)}\n允许动作：${JSON.stringify(allowed)}\n剩余可花云贝预算：${(body.max_spend||30)-spent}。\n只返回 JSON：{"action":"白名单动作","amount":1,"target":"需要时填写","comment":"一句当轮想法"}。`;
        try{const provider=providers().find(item=>item.id===body.provider_id),request={provider_id:body.provider_id,messages:[{role:"user",content:instruction}],system:"",max_tokens:96,temperature:.2,thinking_enabled:false},answer=native?await nativeChat(request):await webChat(request),match=(answer.content||"").match(/\{[\s\S]*\}/),choice=match?JSON.parse(match[0]):{},fallback=fallbackAiGameAction(gameId,current,(body.max_spend||30)-spent),valid=allowed.find(item=>item.action===choice.action&&(!item.target||item.target===choice.target))||fallback;if(!valid)throw new Error("当前局面没有可安全执行的游戏动作");const cost=gameId==="claw_machine"&&valid.action==="grab"?10:gameId==="cloud_slots"?5:gameId==="quiet_fishing"&&valid.action==="buy_bait"?25:0;if(spent+cost>(body.max_spend||30))break;const response=await window.fetch(`/api/games/${gameId}/action`,{method:"POST",body:JSON.stringify(valid)});if(!response.ok)throw new Error((await response.json()).detail);const played=await response.json(),comment=String(choice.comment||(!match?"模型未返回标准动作，已按当前局面安全继续。":"")).slice(0,160),personaName=persona?.name?.trim()||"当前人格";spent+=cost;if(comment){played.state.journal=[...played.state.journal,`${personaName} · 心里话：${comment}`].slice(-30);write(key,played.state);}finalState=played.state;decisions.push({choice:valid,comment,events:played.events});}catch(error){return json({detail:error.message},502);}
