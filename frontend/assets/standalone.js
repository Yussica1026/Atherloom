(() => {
  ...
  const webChat = async request => {
    ...
    return {ok:true,content:content||"",model:provider.model,tool_calls:toolCalls,raw_assistant:rawAssistant};
  };
  ...
  const discoverRemoteMcp=async server=>{if(server.transport==="stdio")throw new Error("Android 不支持本机 stdio MCP");let result=await mcpPost(server,"initialize",{protocolVersion:"2025-03-26",capabilities:{},clientInfo:{name:"Atherloom Android",version:"0.5.20"}}),session=result.session_id||"";await mcpPost(server,"notifications/initialized",undefined,null,session);result=await mcpPost(server,"tools/list",{},2,session);return result.response?.result?.tools||[];};
  const callRemoteMcp=async(server,name,args)=>{let result=await mcpPost(server,"initialize",{protocolVersion:"2025-03-26",capabilities:{},clientInfo:{name:"Atherloom Android",version:"0.5.20"}}),session=result.session_id||"";await mcpPost(server,"notifications/initialized",undefined,null,session);result=await mcpPost(server,"tools/call",{name,arguments:args||{}},3,session);return result.response?.result||{};};
      ...
      try {
        ...
        if(tools.length){
          ...
          if(detectedCalls.length){
            ...
            const final=native?await nativeChat(request):await webChat(request),output=persistStreamEvent({delta:final.content||""});
            return ndjson([...output,...persistStreamEvent({done:true})]);
          }
          const output=persistStreamEvent({delta:probe.content||""});
          return ndjson([...output,...persistStreamEvent({done:true})]);
        }
