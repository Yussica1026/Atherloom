...
await check("Android non-streaming chat uses an asynchronous bridge", `(async()=>{const source=await fetch('assets/standalone.js?v=0424').then(response=>response.text());return source.includes('AtherloomNativeResolve')&&source.includes('await nativeChat(request)')})()`);
await check("AI game decisions use lightweight model settings", `(async()=>{const source=await fetch('assets/standalone.js?v=0424').then(response=>response.text());return source.includes('max_tokens:96')&&source.includes('thinking_enabled:false')})()`);
await check("AI game falls back to a safe local action", `(async()=>{const source=await fetch('assets/standalone.js?v=0424').then(response=>response.text());return source.includes('fallbackAiGameAction')&&source.includes('||fallback')})()`);
