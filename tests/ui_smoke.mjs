...
await check("stream backlog drains adaptively", `(async()=>{const original=state.messages,message={role:'assistant',content:'',reasoning:'',pending:true,streaming:true};state.messages=[message];renderMessages();const presenter=createStreamPresenter(message,true),full='长'.repeat(600),started=Date.now();presenter.push(full);await presenter.finish();const ok=message.content===full&&Date.now()-started<900;state.messages=original;renderMessages();return ok})()`);
...
await check("Android non-streaming chat uses an asynchronous bridge", `(async()=>{const source=await fetch('assets/standalone.js?v=0423').then(response=>response.text());return source.includes('AtherloomNativeResolve')&&source.includes('await nativeChat(request)')})()`);
await check("AI game decisions use lightweight model settings", `(async()=>{const source=await fetch('assets/standalone.js?v=0423').then(response=>response.text());return source.includes('max_tokens:96')&&source.includes('thinking_enabled:false')})()`);
